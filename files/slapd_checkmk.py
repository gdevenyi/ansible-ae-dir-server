...
# Special characters which have to be masked in check item names
ITEM_NAME_SPECIAL_CHARS = '!:$%=\\'
...
#        syncrepl_list = []
        ...
        try:
            ...
            for db_num, db_suffix, db_type, db_dir in db_suffixes:
                ...
                try:
                    ...
                    noop_result = ldap_conn.noop_search_st(
                        db_suffix,
        ...
        for db_num, db_suffix, _ in syncrepl_list:
            ...
            self.add_item(item_name)
            try:
                local_csn_dict[db_suffix] = ldap_conn.get_context_csn(db_suffix)
            except CatchAllException, exc:
                self.result(
                    CHECK_RESULT_ERROR,
                    item_name,
                    ...
                    if not remote_csn_dict[syncrepl_target_uri][db_suffix]:
                        ...
                        self.result(
                            ...
                            check_output='%d contextCSN attribute values retrieved for %r from %r' % (
                                len(remote_csn_dict[syncrepl_target_uri][db_suffix]),
                                db_suffix,
                                ldap_conn._uri,
                            )
                        )
